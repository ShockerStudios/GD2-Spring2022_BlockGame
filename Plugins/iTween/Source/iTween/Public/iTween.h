...
private:
	...
	UFUNCTION()
		static void StopTweens_Internal(TArray<AiTweenEvent*> tweens);
...
public:
	...
	UFUNCTION(BlueprintPure, meta = (UnsafeDuringActorConstruction), Category = "iTween|Utilities")
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Utilities")
		static FVector ConstrainVector(FVector inputVector, FVector currentVector, EVectorConstraints::ItweenVectorConstraints vectorConstraints);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Utilities")
		static FVector2D ConstrainVector2D(FVector2D inputVector2D, FVector2D currentVector2D, EVector2DConstraints::ItweenVector2DConstraints vector2DConstraints);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Utilities")
		static FRotator ConstrainRotator(FRotator inputRotator, FRotator currentRotator, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints);

	//Stopping, Pausing and Resuming Tweens
	/**
	* Stops a tween based on its index in the Aux master object's list of running tweens. These indices are assigned in order of creation. Be careful, after a tween is ended, it's destroyed and removed from the Aux's list of tweens so putting in a hard number won't always stop the tween you expect it to. Relatively unsafe if you're not keeping track. Try another Stopping method for something that may better suit your expectations.
	* @param index The associated index of the tween. If this is your first and only tween, its index in 0. Your second tween is 1, assuming neither tween has ended yet.
	...
	*/
	...
	/**
	* Stops a tween based on the name of the object it's acting upon. Of course this does not apply to data tweens or any tweens whose object has been destroyed when the boolean TieToObjectValidity is set to false.
	* @param objectName Name of the object the tween is acting upon, should it still exist. Exact matches only.
	* @param AffectAllMatching If true, will stop all tweens acting upon an object with the matching name even if they're different objects/tweens. If false, it will stop at the first matching tween it finds. True by default.

	*/
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void StopTweeningByTweeningObjectName(FName objectName = NAME_None, bool AffectAllMatching = true);

	/**
	* Stops a tween based on its given TweenName. If you have not assigned a TweenName, it will not be affected since there is nothing to match.
	* @param tweenName Name of the tween, e.g. its TweenName, an optional parameter set when the tween was created. Exact matches only.
	* @param AffectAllMatching If true, will stop all tweens with the matching name even if they're different tweens. If false, it will stop at the first matching tween it finds. True by default.

	*/
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void StopTweeningByTweenName(FName tweenName = NAME_None, bool AffectAllMatching = true);

	/**
	* Stops a tween based on a reference to the object it's acting upon. Similar to StopTweeningByTweeningObjectName but using an object reference. Of course this does not apply to data tweens or any tweens whose object has been destroyed when the boolean TieToObjectValidity is set to false.
	* @param object A reference to the object the tween is acting upon.
	* @param AffectAllMatching If true, will stop all tweens acting upon an object with the matching reference even if they're different objects/tweens. If false, it will stop at the first tween it finds. True by default.

	*/
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void StopTweeningByTweeningObjectReference(UObject* object = nullptr, bool AffectAllMatching = true);

	/**
	* Stops a tween based on a reference to the iTweenEventObject generated when a tween is created (the return object). There can only be one iTweenObject per tween, so this will affect only one tween.
	* @param object A reference to an iTweenEvent object that you want to stop.
	...
	*/
	...
	/**
	* Stops all tweens in the Aux master object's list of running tweens. 
	...
	*/
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void PauseTweeningByTweeningObjectName(FName objectName = "No Name", bool AffectAllMatching = true);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void PauseTweeningByTweenName(FName tweenName = "No Name", bool AffectAllMatching = true);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void ResumeTweeningByTweeningObjectName(FName objectName = "No Name", bool AffectAllMatching = true);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void ResumeTweeningByTweenName(FName tweenName = "No Name", bool AffectAllMatching = true);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void RestartTweeningByIndex(int32 index);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void RestartTweeningByTweeningObjectName(FName objectName = "No Name", bool AffectAllMatching = true);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void RestartTweeningByTweenName(FName tweenName = "No Name", bool AffectAllMatching = true);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void RestartTweeningByTweeningObjectReference(UObject* object);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void RestartTweeningByEventReference(AiTweenEvent* object);

	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Stopping, Pausing and Resuming Tweens")
		static void RestartAllTweens();
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Getting Event Operators")
		static TArray<AiTweenEvent*> GetAllTweeningEvents();
	...
	*/
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Actor Move From/To (Simple)"), Category = "iTween|Events|Actor Tweens|Actor Move")
		static AiTweenEvent* ActorMoveFromToSimple(FName tweenName = "No Name", AActor* actorToMove = nullptr, FVector locationFrom = FVector::ZeroVector, FVector locationTo = FVector::ZeroVector, ECoordinateSpace::ItweenCoordinateSpace coordinateSpace = ItweenCoordinateSpace::world, bool sweep = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	*/
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Actor Move To Spline Point (Simple)"), Category = "iTween|Events|Actor Tweens|Actor Move")
		static AiTweenEvent* ActorMoveToSplinePointSimple(FName tweenName = "No Name", AActor* actorToMove = nullptr, USplineComponent* splineComponent = nullptr, bool moveToPath = false, bool sweep = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr, bool destroySplineComponent = false);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Events|Actor Tweens|Actor Move")
		static void ActorMoveUpdate(AActor* actorToMove, FVector locationTo, float delta, float speed = 5.f, bool isLocal = false, bool sweep = false, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none, ELookType::ItweenLookType orientation = noOrientationChange, UObject* orientationTarget = nullptr, float orientationSpeed = 5.0f, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints = ItweenRotatorConstraints::none);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Actor Rotate From/To (Simple)"), Category = "iTween|Events|Actor Tweens|Actor Rotate")
		static AiTweenEvent* ActorRotateFromToSimple(FName tweenName = "No Name", AActor* actorToRotate = nullptr, FRotator rotationFrom = FRotator::ZeroRotator, FRotator rotationTo = FRotator::ZeroRotator, ECoordinateSpace::ItweenCoordinateSpace coordinateSpace = ItweenCoordinateSpace::world, bool shortestPath = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Events|Actor Tweens|Actor Rotate")
		static void ActorRotateUpdate(AActor* actorToRotate, FRotator rotationTo, float delta, float speed = 5.f, bool isLocal = false, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints = ItweenRotatorConstraints::none);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Actor Scale From/To (Full)"), Category = "iTween|Events|Actor Tweens|Actor Scale")
		static AiTweenEvent* ActorScaleFromToFull(float timerInterval = 0.f, FName tweenName = "No Name", AActor* actorToScale = nullptr, FVector scaleFrom = FVector::ZeroVector, FVector scaleTo = FVector(1, 1, 1), bool enforceValueTo = true, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none, bool isLocal = false, float delay = 0.0f, EDelayType::ItweenDelayType delayType = first, ETickType::ItweenTickType tickType = seconds, float tickTypeValue = 5.0f, EEaseType::ItweenEaseType easeType = linear, UCurveFloat* customEaseTypeCurve = nullptr, float punchAmplitude = 1.0f, ELoopType::ItweenLoopType loopType = once, int32 maximumLoopSections = 0, UObject* onTweenStartTarget = nullptr, FString OnTweenStartFunctionName = "", UObject* onTweenUpdateTarget = nullptr, FString OnTweenUpdateFunctionName = "", UObject* onTweenLoopTarget = nullptr, FString OnTweenLoopFunctionName = "", UObject* onTweenCompleteTarget = nullptr, FString OnTweenCompleteFunctionName = "", bool tickWhenPaused = false, bool ignoreTimeDilation = false, bool tieToObjectValidity = true, bool cullNonRenderedTweens = false, float secondsToWaitBeforeCull = 3.f);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Actor Scale From/To (Simple)"), Category = "iTween|Events|Actor Tweens|Actor Scale")
		static AiTweenEvent* ActorScaleFromToSimple(FName tweenName = "No Name", AActor* actorToScale = nullptr, FVector scaleFrom = FVector::ZeroVector, FVector scaleTo = FVector(1, 1, 1), bool isLocal = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Events|Actor Tweens|Actor Scale")
		static void ActorScaleUpdate(AActor* actorToScale, FVector scaleTo, float delta, float speed = 5.f, bool isLocal = false, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none);
	//Component
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Component Move From/To (Full)"), Category = "iTween|Events|Component Tweens|Component Move")
		static AiTweenEvent* ComponentMoveFromToFull(float timerInterval = 0.f, FName tweenName = "No Name", USceneComponent* componentToMove = nullptr, FVector locationFrom = FVector::ZeroVector, FVector locationTo = FVector::ZeroVector, bool enforceValueTo = true, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none, ECoordinateSpace::ItweenCoordinateSpace coordinateSpace = ItweenCoordinateSpace::world, bool sweep = false, float delay = 0.0f, EDelayType::ItweenDelayType delayType = first, ETickType::ItweenTickType tickType = seconds, float tickTypeValue = 5.0f, EEaseType::ItweenEaseType easeType = linear, UCurveFloat* customEaseTypeCurve = nullptr, float punchAmplitude = 1.0f, ELoopType::ItweenLoopType loopType = once, int32 maximumLoopSections = 0, ELookType::ItweenLookType orientation = noOrientationChange, UObject* orientationTarget = nullptr, float orientationSpeed = 5.0f, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints = ItweenRotatorConstraints::none, UObject* onTweenStartTarget = nullptr, FString OnTweenStartFunctionName = "", UObject* onTweenUpdateTarget = nullptr, FString OnTweenUpdateFunctionName = "", UObject* onTweenLoopTarget = nullptr, FString OnTweenLoopFunctionName = "", UObject* onTweenCompleteTarget = nullptr, FString OnTweenCompleteFunctionName = "", bool tickWhenPaused = false, bool ignoreTimeDilation = false, bool tieToObjectValidity = true, bool cullNonRenderedTweens = false, float secondsToWaitBeforeCull = 3.f);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Component Move From/To (Simple)"), Category = "iTween|Events|Component Tweens|Component Move")
		static AiTweenEvent* ComponentMoveFromToSimple(FName tweenName = "No Name", USceneComponent* componentToMove = nullptr, FVector locationFrom = FVector::ZeroVector, FVector locationTo = FVector::ZeroVector, ECoordinateSpace::ItweenCoordinateSpace coordinateSpace = ItweenCoordinateSpace::world, bool sweep = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Component Move To Spline Point (Full)"), Category = "iTween|Events|Component Tweens|Component Move")
		static AiTweenEvent* ComponentMoveToSplinePointFull(float timerInterval = 0.f, FName tweenName = "No Name", USceneComponent* componentToMove = nullptr, USplineComponent* splineComponent = nullptr, bool moveToPath = false, bool enforceValueTo = true, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none, bool sweep = false, float delay = 0.0f, EDelayType::ItweenDelayType delayType = first, ETickType::ItweenTickType tickType = seconds, float tickTypeValue = 5.0f, EEaseType::ItweenEaseType easeType = linear, UCurveFloat* customEaseTypeCurve = nullptr, float punchAmplitude = 1.0f, ELoopType::ItweenLoopType loopType = once, int32 maximumLoopSections = 0, bool switchPathOrientationDirection = true, ELookType::ItweenLookType orientation = noOrientationChange, UObject* orientationTarget = nullptr, float orientationSpeed = 5.0f, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints = ItweenRotatorConstraints::none, UObject* onTweenStartTarget = nullptr, FString OnTweenStartFunctionName = "", UObject* onTweenUpdateTarget = nullptr, FString OnTweenUpdateFunctionName = "", UObject* onTweenLoopTarget = nullptr, FString OnTweenLoopFunctionName = "", UObject* onTweenCompleteTarget = nullptr, FString OnTweenCompleteFunctionName = "", bool tickWhenPaused = false, bool ignoreTimeDilation = false, bool destroySplineComponent = false, bool tieToObjectValidity = true, bool cullNonRenderedTweens = false, float secondsToWaitBeforeCull = 3.f);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Component Move To Spline Point (Simple)"), Category = "iTween|Events|Component Tweens|Component Move")
		static AiTweenEvent* ComponentMoveToSplinePointSimple(FName tweenName = "No Name", USceneComponent* componentToMove = nullptr, USplineComponent* splineComponent = nullptr, bool moveToPath = false, bool sweep = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr, bool destroySplineComponent = false);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Events|Component Tweens|Component Move")
		static void ComponentMoveUpdate(USceneComponent* componentToMove, FVector locationTo, float delta, float speed = 5.f, bool isLocal = false, bool sweep = false, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none, ELookType::ItweenLookType orientation = noOrientationChange, UObject* orientationTarget = nullptr, float orientationSpeed = 5.0f, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints = ItweenRotatorConstraints::none);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Component Rotate From/To (Simple)"), Category = "iTween|Events|Component Tweens|Component Rotate")
		static AiTweenEvent* ComponentRotateFromToSimple(FName tweenName = "No Name", USceneComponent* componentToRotate = nullptr, FRotator rotationFrom = FRotator::ZeroRotator, FRotator rotationTo = FRotator::ZeroRotator, ECoordinateSpace::ItweenCoordinateSpace coordinateSpace = ItweenCoordinateSpace::world, bool shortestPath = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Events|Component Tweens|Component Rotate")
		static void ComponentRotateUpdate(USceneComponent* componentToRotate, FRotator rotationTo, float delta, float speed = 5.f, bool isLocal = false, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints = ItweenRotatorConstraints::none);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Component Scale From/To (Full)"), Category = "iTween|Events|Component Tweens|Component Scale")
		static AiTweenEvent* ComponentScaleFromToFull(float timerInterval = 0.f, FName tweenName = "No Name", USceneComponent* componentToScale = nullptr, FVector scaleFrom = FVector::ZeroVector, FVector scaleTo = FVector(1, 1, 1), bool enforceValueTo = true, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none, bool isLocal = false, float delay = 0.0f, EDelayType::ItweenDelayType delayType = first, ETickType::ItweenTickType tickType = seconds, float tickTypeValue = 5.0f, EEaseType::ItweenEaseType easeType = linear, UCurveFloat* customEaseTypeCurve = nullptr, float punchAmplitude = 1.0f, ELoopType::ItweenLoopType loopType = once, int32 maximumLoopSections = 0, UObject* onTweenStartTarget = nullptr, FString OnTweenStartFunctionName = "", UObject* onTweenUpdateTarget = nullptr, FString OnTweenUpdateFunctionName = "", UObject* onTweenLoopTarget = nullptr, FString OnTweenLoopFunctionName = "", UObject* onTweenCompleteTarget = nullptr, FString OnTweenCompleteFunctionName = "", bool tickWhenPaused = false, bool ignoreTimeDilation = false, bool tieToObjectValidity = true, bool cullNonRenderedTweens = false, float secondsToWaitBeforeCull = 3.f);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Component Scale From/To (Simple)"), Category = "iTween|Events|Component Tweens|Component Scale")
		static AiTweenEvent* ComponentScaleFromToSimple(FName tweenName = "No Name", USceneComponent* componentToScale = nullptr, FVector scaleFrom = FVector::ZeroVector, FVector scaleTo = FVector(1, 1, 1), bool isLocal = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction), Category = "iTween|Events|Component Tweens|Component Scale")
		static void ComponentScaleUpdate(USceneComponent* componentToScale, FVector scaleTo, float delta, float speed = 5.f, bool isLocal = false, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "UMG RT Move From/To (Simple)"), Category = "iTween|Events|UMG Render Transform Tweens|UMG RT Move (Translate)")
		static AiTweenEvent* UMGRTMoveFromToSimple(FName tweenName = "No Name", UWidget* widgetToMove = nullptr, FVector2D locationFrom = FVector2D::ZeroVector, FVector2D locationTo = FVector2D::ZeroVector, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "UMG RT Move Update"), Category = "iTween|Events|UMG Render Transform Tweens|UMG RT Move (Translate)")
		static void UMGRTMoveUpdate(UWidget* widgetToMove, FVector2D locationTo, float delta, float speed = 5.f, EVector2DConstraints::ItweenVector2DConstraints vector2DConstraints = ItweenVector2DConstraints::none/*, ELookType::ItweenLookType orientation = noOrientationChange, UObject* orientationTarget = nullptr, float orientationSpeed = 5.0f, */);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "UMG RT Rotate From/To (Simple)"), Category = "iTween|Events|UMG Render Transform Tweens|UMG RT Rotate (Angle)")
		static AiTweenEvent* UMGRTRotateFromToSimple(FName tweenName = "No Name", UWidget* widgetToRotate = nullptr, float angleFrom = 0, float angleTo = 90, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "UMG RT Scale From/To (Simple)"), Category = "iTween|Events|UMG Render Transform Tweens|UMG RT Scale")
		static AiTweenEvent* UMGRTScaleFromToSimple(FName tweenName = "No Name", UWidget* widgetToScale = nullptr, FVector2D scaleFrom = FVector2D(1, 1), FVector2D scaleTo = FVector2D(2, 2), float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "UMG RT Scale Update"), Category = "iTween|Events|UMG Render Transform Tweens|UMG RT Scale")
		static void UMGRTScaleUpdate(UWidget* widgetToScale, FVector2D scaleTo, float delta, float speed = 5.f, EVector2DConstraints::ItweenVector2DConstraints vector2DConstraints = ItweenVector2DConstraints::none);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "UMG RT Shear From/To (Simple)"), Category = "iTween|Events|UMG Render Transform Tweens|UMG RT Shear")
		static AiTweenEvent* UMGRTShearFromToSimple(FName tweenName = "No Name", UWidget* widgetToShear = nullptr, FVector2D shearFrom = FVector2D::ZeroVector, FVector2D shearTo = FVector2D(5, 5), float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "UMG RT Shear Update"), Category = "iTween|Events|UMG Render Transform Tweens|UMG RT Shear")
		static void UMGRTShearUpdate(UWidget* widgetToShear, FVector2D shearTo, float delta, float speed = 5.f, EVector2DConstraints::ItweenVector2DConstraints vector2DConstraints = ItweenVector2DConstraints::none);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Vector From/To (Full)"), Category = "iTween|Events|Data Tweens|Vector From/To")
		static AiTweenEvent* VectorFromToFull(float timerInterval = 0.f, FName tweenName = "No Name", FVector startValue = FVector::ZeroVector, FVector endValue = FVector::ZeroVector, bool enforceValueTo = true, EVectorConstraints::ItweenVectorConstraints vectorConstraints = ItweenVectorConstraints::none, float delay = 0.0f, EDelayType::ItweenDelayType delayType = first, ETickType::ItweenTickType tickType = seconds, float tickTypeValue = 5.0f, EEaseType::ItweenEaseType easeType = linear, UCurveFloat* customEaseTypeCurve = nullptr, float punchAmplitude = 1.0f, ELoopType::ItweenLoopType loopType = once, int32 maximumLoopSections = 0, UObject* onTweenStartTarget = nullptr, FString OnTweenStartFunctionName = "", UObject* onTweenUpdateTarget = nullptr, FString OnTweenUpdateFunctionName = "", UObject* onTweenLoopTarget = nullptr, FString OnTweenLoopFunctionName = "", UObject* onTweenCompleteTarget = nullptr, FString OnTweenCompleteFunctionName = "", bool tickWhenPaused = false, bool ignoreTimeDilation = false);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Vector From/To (Simple)"), Category = "iTween|Events|Data Tweens|Vector From/To")
		static AiTweenEvent* VectorFromToSimple(FName tweenName = "No Name", FVector startValue = FVector::ZeroVector, FVector endValue = FVector::ZeroVector, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Rotator From/To (Full)"), Category = "iTween|Events|Data Tweens|Rotator From/To")
		static AiTweenEvent* RotatorFromToFull(float timerInterval = 0.f, FName tweenName = "No Name", FRotator startValue = FRotator::ZeroRotator, FRotator endValue = FRotator::ZeroRotator, bool enforceValueTo = true, ERotatorConstraints::ItweenRotatorConstraints rotatorConstraints = ItweenRotatorConstraints::none, bool shortestPath = false, float delay = 0.0f, EDelayType::ItweenDelayType delayType = first, ETickType::ItweenTickType tickType = seconds, float tickTypeValue = 5.0f, EEaseType::ItweenEaseType easeType = linear, UCurveFloat* customEaseTypeCurve = nullptr, float punchAmplitude = 1.0f, ELoopType::ItweenLoopType loopType = once, int32 maximumLoopSections = 0, UObject* onTweenStartTarget = nullptr, FString OnTweenStartFunctionName = "", UObject* onTweenUpdateTarget = nullptr, FString OnTweenUpdateFunctionName = "", UObject* onTweenLoopTarget = nullptr, FString OnTweenLoopFunctionName = "", UObject* onTweenCompleteTarget = nullptr, FString OnTweenCompleteFunctionName = "", bool tickWhenPaused = false, bool ignoreTimeDilation = false);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Rotator From/To (Simple)"), Category = "iTween|Events|Data Tweens|Rotator From/To")
		static AiTweenEvent* RotatorFromToSimple(FName tweenName = "No Name", FRotator startValue = FRotator::ZeroRotator, FRotator endValue = FRotator::ZeroRotator, bool shortestPath = false, float timeInSeconds = 5.0f, EEaseType::ItweenEaseType easeType = linear, FString parameters = "", UObject* onTweenStartTarget = nullptr, UObject* onTweenUpdateTarget = nullptr, UObject* onTweenLoopTarget = nullptr, UObject* onTweenCompleteTarget = nullptr);
	...
	UFUNCTION(BlueprintCallable, meta = (UnsafeDuringActorConstruction, DisplayName = "Linear Color From/To (Full)"), Category = "iTween|Events|Data Tweens|Linear Color From/To")
		static AiTweenEvent* LinearColorFromToFull(float timerInterval = 0.f, FName tweenName = "No Name", FLinearColor startValue = FLinearColor::White, FLinearColor endValue = FLinearColor::Black, bool enforceValueTo = true, float delay = 0.0f, EDelayType::ItweenDelayType delayType = first, ETickType::ItweenTickType tickType = seconds, float tickTypeValue = 5.0f, EEaseType::ItweenEaseType easeType = linear, UCurveFloat* customEaseTypeCurve = nullptr, float punchAmplitude = 1.0f, ELoopType::ItweenLoopType loopType = once, int32 maximumLoopSections = 0, UObject* onTweenStartTarget = nullptr, FString OnTweenStartFunctionName = "", UObject* onTweenUpdateTarget = nullptr, FString OnTweenUpdateFunctionName = "", UObject* onTweenLoopTarget = nullptr, FString OnTweenLoopFunctionName = "", UObject* onTweenCompleteTarget = nullptr, FString OnTweenCompleteFunctionName = "", bool tickWhenPaused = false, bool ignoreTimeDilation = false);
